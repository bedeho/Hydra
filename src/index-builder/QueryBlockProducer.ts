...
export default class QueryBlockProducer extends EventEmitter {
    ...
    async start(at_block?: number) {
        ...
        if(this._started)
            throw Error(`Cannot start when already started.`)

        // mark as started
        this._started = true

        // Try to get initial header right away
        this._height_of_chain = await this._query_service.getFinalizedHead()
            .then((hash) => {
        ...
        if(at_block) {
            ...
            if(this._height_of_chain < at_block)
        ...
        if(!this._producing_blocks_blocks)
            this._produce_blocks();
    }

    async stop() {

        if(!this._started)
            throw new Error(`Cannot stop when not already started.`);
    ...
    private async _produce_blocks() {
        ...
        while (this._block_to_be_produced_next <= this._height_of_chain) {
            ...
            let query_events: QueryEvent[] = records.map((record, index): QueryEvent => {
